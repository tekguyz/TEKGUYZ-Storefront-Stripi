          ...
          {TRUST_ITEMS.map((item, index) => (
            <div key={index} className="flex flex-col items-center relative w-full justify-center">
              <div className="flex flex-col items-center text-center">
                <span className="text-[10px] font-sans font-semibold uppercase tracking-[0.15em] text-[var(--color-text-muted)] mb-1">{item.label}</span>
                <span className="text-base font-light uppercase tracking-[-0.01em] text-[var(--color-text)]">{item.value}</span>
              </div>
